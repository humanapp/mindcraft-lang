...
// `SimEnvironmentStore` cannot be value-imported in this test process: the
// module unconditionally imports `sim-embedded-extensions.ts`, which imports
...
/** apps/sim's `getBrainDiagnostics` composition: brain error diagnostics, then broken-tile compile diagnostics. */
function simBrainDiagnostics(brain: BrainDef, lookup: TileCompileDiagnosticsLookup) {
...
describe("sim per-brain diagnostics feed: broken user-tile compile diagnostics", () => {
  it("surfaces a used broken tile's verbatim diagnostics, deduped, and clears/repopulates on recompile", async () => {
    ...
    try {
      ...
      const feed = simBrainDiagnostics(brainUsing, lookup);
      ...
      assert.deepEqual(simBrainDiagnostics(brainClean, lookup), [], "a brain not using the broken tile gets no entry");
      ...
      assert.equal(
        simBrainDiagnostics(brainDup, lookup).length,
      ...
      assert.deepEqual(simBrainDiagnostics(brainUsing, lookup), [], "the feed clears once the tile compiles cleanly");
      ...
      assert.equal(
        simBrainDiagnostics(brainUsing, lookup).length,
