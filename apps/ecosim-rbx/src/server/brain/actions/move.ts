...
function resolveAwayFromTarget(ctx: ExecutionContext, args: ReadonlyList<Value>): Vector2 | undefined {
  ...
  if (isListValue(targetPositionsVar)) {
    ...
    const sx = selfActor?.sprite.x ?? 0;
    const sy = selfActor?.sprite.y ?? 0;
    ...
    let best1Idx = -1;
    let best1Dist = math.huge;
    let best2Idx = -1;
    let best2Dist = math.huge;
    for (let i = 0; i < allPositions.size(); i++) {
      const pos = allPositions.get(i);
      const dx = pos.X - sx;
      const dy = pos.Y - sy;
...
function computeWanderSteering(
  ...
  if (!state || state.wanderTargetExpiresAt < now) {
    state = {
      wanderTargetPos: mkVector2Value(selfActor.randomPosition()),
  ...
  return steerToward(selfActor, targetPos, weight, speedMultiplier);
}
...
function computeSteering(ctx: ExecutionContext, args: ReadonlyList<Value>, selfActor: Actor): Steering | undefined {
  ...
  if (hasArg(args, kWanderSlotId)) {
    return computeWanderSteering(ctx, selfActor, weight, speedMultiplier);
  }

  if (hasArg(args, kTowardSlotId)) {
    const targetPos = resolveTargetPosition(ctx, args, kAnonActorRefSlotId);
    return targetPos ? steerToward(selfActor, targetPos, weight, speedMultiplier) : undefined;
  }

  if (hasArg(args, kAwayFromSlotId)) {
    const targetPos = resolveAwayFromTarget(ctx, args);
    return targetPos ? steerAwayFrom(selfActor, targetPos, weight, speedMultiplier) : undefined;
  }

  if (hasArg(args, kAvoidSlotId)) {
    const targetPos = resolveTargetPosition(ctx, args, kAnonActorRefSlotId);
    return targetPos ? steerAvoid(selfActor, targetPos, weight, speedMultiplier) : undefined;
  }

  // Default: move forward (includes explicit Forward modifier or no directional modifier)
  return steerForward(selfActor, weight, speedMultiplier);
}
...
/** Modifier tiles the `move` actuator accepts. */
export const modifiers: ModifierTileInput[] = [
  { id: TileIds.Modifier.MovementForward, label: "forward" },
...
/** Parameter tiles the `move` actuator accepts. */
