...
function fakeStore(env: MindcraftEnvironment, defs: Record<Archetype, BrainDef>): SimEnvironmentStore {
  return {
    env,
...
describe("engine re-instantiates living actor brains on recompile", () => {
  test("an actor spawned while a tile is uninstalled recovers when the tile is re-added", async () => {
    const env = createMindcraftEnvironment({ modules: [coreModule(), createSimModule()] });
