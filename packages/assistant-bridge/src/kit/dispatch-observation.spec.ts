...
/** The rule that emits: the one a fresh brain document already holds. */
const kEmitRule = "0/0";

/** The rule that chimes, added beside the first. */
const kChimeRule = "0/1";
 ...
 */
async function rehearseBothDispatchKinds(thinks: number): Promise<SimulationRun> {
  ...
  place(workspace, kEmitRule, "when", [`tile.sensor->${FakeActionKeys.Signal}`]);
  place(workspace, kEmitRule, "do", [`tile.actuator->${FakeActionKeys.Emit}`, `tile.modifier->${FakeTileIds.Loudly}`]);
  place(workspace, kChimeRule, "when", [`tile.sensor->${FakeActionKeys.Signal}`]);
  place(workspace, kChimeRule, "do", [
...
describe("a rehearsal's dispatch observations", () => {
  ...
  test("renders an asynchronous call's arguments and attributes it to its rule", async () => {
    ...
    assert.equal(chime.ruleId, kChimeRule);
