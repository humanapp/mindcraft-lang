...
describe("buildSimCatalogOffers -- compatibility-filtered against the sim stack", () => {
...
describe("buildSimExtensionEntries -- manifest-map membership drives gh: cards", () => {
  ...
  test("lists a top-level gh: install from the map and omits a transitive gh: dep held only in the snapshot store", () => {
    const extensions = { [SIM_LIB_COORDINATE]: SIM_LIB_REFERENCE, [TOP_LIB]: topRef };
    ...
    const entries = buildSimExtensionEntries(extensions, [simLayer], installedContent);
