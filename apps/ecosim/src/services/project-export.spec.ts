...
import { buildEcosimExportDocument, type EcosimAppChunk } from "./project-io";
...
describe("sim project export document", () => {
  it("carries the whole project: tile source, asset, brain, and the sim's app chunk", async () => {
    ...
    const document = JSON.parse(await buildEcosimExportDocument(host.projectManager, { herbivore: 5 }, OBSTACLES)) as {
      manifest: {
    ...
    const app = document.manifest.app[simName] as EcosimAppChunk;
