...
export function computeTileBadges(
  ...
  if (firstExpr.kind === "errorExpr") {
    // Entire first expression is an error -- mark its tiles as errors
    markSpanWithBadge(badges, firstExpr.span, "error", "Parse error");
  } else {
    // First expression is the main one -- walk it for incomplete sub-expressions
    acceptExprVisitor(firstExpr, new WarningCollector(badges));
  }

  // All subsequent expressions are error-recovery artifacts
  for (let i = 1; i < exprs.size(); i++) {
    const expr = exprs.get(i);
    markAllTiles(expr, badges, "error", "Unexpected expression");
  }

  // Apply parse diagnostics as badge messages. Each diagnostic targets a single
  // tile: the first tile in its span that hasn't already been claimed by a
  // narrower diagnostic. Tiles with an existing AST-level badge get their
  // message refined; tiles without one receive a new warning badge. Empty spans
  // (pointing past the last tile) attach to the preceding tile instead.
  ...
  parseResult.diags.forEach((diag) => {
    ...
    if (width === 0 && diag.span.from > 0) {
      // Empty span (e.g. end-of-input) -- apply to the preceding tile
      applyDiag(diag.span.from - 1, width, msg);
    } else {
      // Apply to the first tile that accepts it, then stop
  ...
  // Apply type diagnostics as warning badges on the node's span.
  // DataTypeConverted is informational, not a problem -- skip it.
...
function setBadge(badges: Map<number, TileBadge>, tileIndex: number, type: "error" | "warning", message: string): void {
  const existing = badges.get(tileIndex);
  if (existing?.type === "error") return; // never downgrade
...
/**
 * Visitor that marks every tile covered by an expression tree with a fixed badge.
 * Used for error-recovery artifact expressions (position > 0 in parse results).
 */
...
/**
 * Visitor that walks the main (non-error) expression looking for incomplete
 * sub-expressions and marks the "owner" tiles with warning badges.
 *
 * Incomplete patterns:
 * - ParameterExpr whose value is ErrorExpr -> mark the parameter tile
 * - BinaryOpExpr whose right is ErrorExpr -> mark the operator tile
 * - UnaryOpExpr whose operand is ErrorExpr -> mark the operator tile
 * - AssignmentExpr whose value is ErrorExpr -> mark the target tile
 * - Any nested ErrorExpr that wasn't caught above -> mark its span as error
 */
...
/**
 * Visitor that collects all nodeId -> Expr mappings from an expression tree.
 */
