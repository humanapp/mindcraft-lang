...
- A flat run of \`otherwise\` rules is an if / else-if / else ladder: each fires on a think when no earlier rule of the run fired that think and its own expression holds. An empty expression is the plain \`else\`. The \`otherwise\` mode replaces the deprecated \`otherwise\` tile; set the mode instead of placing the tile.
