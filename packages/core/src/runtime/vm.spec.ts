...
import { Dict, List, type ReadonlyList } from "@wendoo/core";
import { BrainServices } from "@wendoo/core/brain";
    ...
    const hostFnEntry = services.runtime.functions.register(
      ...
      false,
      {
        exec: (ctx: ExecutionContext) => {
          seenRuleFuncId = ctx.currentRuleFuncId;
...
describe("VM -- overflow faults", () => {
  test("operand stack overflow surfaces as ErrorCode.StackOverflow", () => {
    ...
    const vm = new VM(prog, toVmServices(services), { handles: new HandleTable(100), maxStackSize: 8 });
    const fiber = vm.spawnFiber(1, 0, List.empty(), mkCtx());
    fiber.instrBudget = 1000;
  ...
  test("handler stack overflow surfaces as ErrorCode.StackOverflow", () => {
    ...
    const vm = new VM(prog, toVmServices(services), { handles: new HandleTable(100), maxHandlers: 8 });
    const fiber = vm.spawnFiber(1, 0, List.empty(), mkCtx());
    fiber.instrBudget = 1000;
...
describe("VM -- operator monomorphization", () => {
  test("primitive number arithmetic does not consult ITypeRegistry on the dispatch hot path", () => {
    ...
    const vm = new VM(prog, toVmServices(countedServices));
    const fiber = vm.spawnFiber(1, 0, List.empty(), mkCtx());
...
describe("VM -- slot-keyed variable dispatch", () => {
  test("LOAD_VAR_SLOT / STORE_VAR_SLOT do not call name-keyed getVariable / setVariable", () => {
    ...
    const counterServices = __test__createPlatformServices({
      brainVars: {
        getByName: () => {
          counter.name++;
          return NIL_VALUE;
...
describe("VM -- VMEvents passivity", () => {
  test("event observer does not affect fiber execution outcome", () => {
    ...
    const vm1 = new VM(prog, toVmServices(services));
    ...
    const vm2 = new VM(prog, toVmServices(services), {
      events: {
        onFiberDone: (payload) => {
