...
`createInMemoryWorkspace` for browser apps:

```typescript
import { createInMemoryWorkspace } from "@mindcraft-lang/bridge-app";
...
`tsconfig.json`) from being stored in the workspace -- these are generated by the compiler
and injected automatically when the workspace snapshot is exported to the bridge.
